...
import { getPatientHistories } from 'apiSdk/patient-histories';
...
function DashboardPage() {
  ...
  const medicalStaffStat = useSWR<PaginatedInterface<MedicalStaffInterface>>(
    () => `/medical-staffs`,
    () => getMedicalStaffs({ limit: 1, offset: 0 }),
  );
  ...
  const patientHistoryStat = useSWR<PaginatedInterface<PatientHistoryInterface>>(
    () => `/patient-histories`,
    () => getPatientHistories({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Medical Staffs',
      result: medicalStaffStat?.data?.totalCount,
      isLoading: medicalStaffStat.isLoading,
    ...
    },
    {
      ...
      result: patientHistoryStat?.data?.totalCount,
      isLoading: patientHistoryStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
